...
.popup-overlay{position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:9999;
  ...
  background:rgba(0,0,0,0.78);animation:fadeInOut 2s forwards;}
...
# ═══════════════════════════════════════════════════════════════════════════════
def render_popup():
    """Render wicket/four popup if triggered"""
...
def page_live():
    ...
    if striker["name"]=="":
        ...
        # All out only if no players left AND wickets >= MAX
        if not avail and inn["wickets"] >= MAX_WICKETS:
            st.error("All out!"); page_end_innings(s,ik); return
        elif not avail:
            # Shouldn't happen — reset striker to non-striker if available
            if inn["batsmen"]["non_striker"]["name"]:
                inn["batsmen"]["striker"] = dict(inn["batsmen"]["non_striker"])
                inn["batsmen"]["non_striker"] = {"name":"","runs":0,"balls":0,"fours":0,"sixes":0}
                push(); st.rerun()
            else:
                st.error("No more batsmen!"); page_end_innings(s,ik); return
...
def render_card(inn,bt,bwt):
    ...
    brows=[]
    # Build combined bowling card: saved completed overs + current bowler spell
    combined = {}
    for n,d in inn["bowling_card"].items():
        combined[n] = {"overs":d["overs"],"balls":d.get("balls",0),"runs":d["runs"],"wickets":d["wickets"]}
    # Add current bowler's in-progress spell
    cb = inn["bowler"]
    if cb.get("name") and cb["balls"] > 0:
        cn = cb["name"]
        if cn in combined:
            combined[cn]["balls"] += cb["balls"]
            combined[cn]["runs"]  += cb["runs"]
            combined[cn]["wickets"] += cb["wickets"]
        else:
            combined[cn] = {"overs":0,"balls":cb["balls"],"runs":cb["runs"],"wickets":cb["wickets"]}
    for n,d in combined.items():
        total_balls = d["overs"]*6 + d["balls"]
        overs_disp = f"{d['overs']}.{d['balls']}"
        ec = round((d["runs"]/total_balls)*6,2) if total_balls>0 else 0
