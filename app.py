...
def render_popup():
    ...
    # Store when popup was triggered (if not already set)
    if "popup_start" not in st.session_state:
        st.session_state["popup_start"] = datetime.now().timestamp()
    # Auto-rerun after 2.2s using JS meta refresh trick
    st.markdown('<meta http-equiv="refresh" content="2">', unsafe_allow_html=True)
    # Check if 2s has passed — if so clear popup
    import time as _time
    elapsed = datetime.now().timestamp() - st.session_state.get("popup_start", datetime.now().timestamp())
    if elapsed >= 2.0:
        st.session_state["popup_type"] = None
        st.session_state.pop("popup_start", None)
        st.rerun()
