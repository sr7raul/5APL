...
def record_ball(inn, runs, extra=None, wicket=False, ot_runs=0):
    ...
    # runs = batsman runs, ot_runs = overthrow extras (not credited to batsman)
    legal = extra not in ("wide","noball")
    bye_type = extra in ("overthrow",)
    ...
    # Bowler not charged for overthrows
    if not bye_type:
    ...
    if st.session_state.get("pending_wicket"):
        ...
        if dismissal:
            ...
            if run_out_non_striker:
                # Non-striker is out — swap them to striker position so record_ball dismisses correctly
                inn["batsmen"]["striker"], inn["batsmen"]["non_striker"] =                     inn["batsmen"]["non_striker"], inn["batsmen"]["striker"]
                sname = inn["batsmen"]["striker"]["name"]
                # Add non-striker to batting card if not already there
                if sname not in inn["batting_card"]:
                    inn["batting_card"][sname] = {"name":sname,"runs":0,"balls":0,"fours":0,"sixes":0,"out":False}
            else:
                sname = striker_name
