...
def record_ball(inn, runs, extra=None, wicket=False, ot_runs=0):
    # runs     = batsman runs between wickets → goes to BATSMAN score
    # ot_runs  = overthrow extras             → goes to TEAM score only (extras)
    # extra: None | "wide" | "noball" | "overthrow"
    legal = extra not in ("wide","noball")
    is_overthrow = extra == "overthrow"

    # Snapshot striker BEFORE any changes (for undo)
    striker_before = dict(inn["batsmen"]["striker"])

    # Total score = batsman runs + overthrow extras (+ wide/noball penalty)
    inn["score"] += runs + ot_runs
    if extra in ("wide","noball"): inn["score"] += 1

    # Bowler charged for batsman runs (off his delivery) — NOT overthrow extras
    # e.g. 2+OT4: bowler charged 2 runs (batsman ran), not the 4 overthrow
    ...
    if wicket and extra != "wide":
        ...
        # Batsman runs ALWAYS go to batsman (even on overthrow)
        if extra not in ("wide",) and runs > 0:
            inn["batsmen"]["striker"]["runs"] += runs
            if runs == 4: inn["batsmen"]["striker"]["fours"] += 1

    # Rotate strike on odd TOTAL runs (batsman runs + ot_runs combined)
    total_for_rotation = runs + ot_runs
    if legal and not wicket and total_for_rotation % 2 == 1:
...
def undo_ball(inn):
    ...
    # Reverse score
    inn["score"] -= b["runs"] + b.get("ot_runs", 0)
    ...
    if b["wicket"] and b["extra"] != "wide":
        ...
        if b["extra"] not in ("wide",) and not b["wicket"]:
            # Batsman runs always credited — reverse them
...
def export_excel(state):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as w:
        ...
        # Sheet: Match-wise Batting Scorecards (clean format like real scorecard)
        bat_mw = []
        bowl_mw = []
        seen = set()
        for mr in state.get("match_results", []):
            ...
            match_label = f"M{mn}: {mr.get('Team 1','')} vs {mr.get('Team 2','')}"
            mom = mr.get("Man of the Match","-")
            result = mr.get("Result","-")
            for inn_label, inn_data in [("Innings 1", i1d), ("Innings 2", i2d)]:
                if not inn_data: continue
                bat_team = inn_data.get("batting_team","")
                bowl_team = inn_data.get("bowling_team","")
                # Batting rows
                for pname, pd_data in inn_data.get("batting_card", {}).items():
                    sr = round((pd_data["runs"]/pd_data["balls"])*100,1) if pd_data.get("balls",0)>0 else 0.0
                    dismissal = pd_data.get("dismissal","not out") if pd_data.get("out") else "not out"
                    bat_mw.append({
                        "Match": match_label, "Stage": mr.get("Stage","League"),
                        "Innings": inn_label, "Batting Team": bat_team,
                        "Batsman": pname, "R": pd_data.get("runs",0),
                        "B": pd_data.get("balls",0), "4s": pd_data.get("fours",0),
                        "SR": sr, "Dismissal": dismissal,
                        "Result": result, "MOM": mom
                    })
                # Bowling rows
                combined_bowl = {}
                for bname, bd in inn_data.get("bowling_card", {}).items():
                    tb = bd.get("overs",0)*6 + bd.get("balls",0)
                    combined_bowl[bname] = {"total_balls":tb,"runs":bd.get("runs",0),"wickets":bd.get("wickets",0)}
                for bname, bd in combined_bowl.items():
                    tb = bd["total_balls"]
                    odisp = ostr(tb)
                    ec = round((bd["runs"]/tb)*6,2) if tb>0 else 0.0
                    bowl_mw.append({
                        "Match": match_label, "Stage": mr.get("Stage","League"),
                        "Innings": inn_label, "Bowling Team": bowl_team,
                        "Bowler": bname, "O": odisp,
                        "R": bd["runs"], "W": bd["wickets"], "Econ": ec,
                        "Result": result, "MOM": mom
                    })
        if bat_mw:
            pd.DataFrame(bat_mw).to_excel(w, sheet_name="Match Batting Cards", index=False)
        if bowl_mw:
            pd.DataFrame(bowl_mw).to_excel(w, sheet_name="Match Bowling Cards", index=False)
...
def page_live():
    ...
    if inn["ball_log"]:
        ...
        for b in inn["ball_log"][-36:]:
            ...
            elif b["extra"]=="overthrow":
                cls="b3"
                bat_r = b["runs"]
                ot_r  = b.get("ot_runs",0)
                lbl   = f"{bat_r}+OT{ot_r}" if bat_r>0 else f"OT{ot_r}"
            else:
    ...
    # Need new batsman? (striker out OR non-striker run out)
    need_striker   = striker["name"] == ""
    need_nonstriker = inn["batsmen"]["non_striker"]["name"] == "" and not need_striker

    if need_striker or need_nonstriker:
        role = "Striker" if need_striker else "Non-Striker (run out replacement)"
        st.warning(f"🚨 Wicket! Select new {role}.")
        used = set()
        for pname, pdata in inn["batting_card"].items():
            if (pdata.get("balls", 0) > 0 or
                pdata.get("out", False) or
                pname == inn["batsmen"]["striker"]["name"] or
                pname == inn["batsmen"]["non_striker"]["name"]):
                used.add(pname)
        used.add(inn["batsmen"]["striker"]["name"])
        ...
        if not avail:
            ...
            return
        nb = st.selectbox(f"New Batsman ({role})", avail)
        if st.button("✅ Confirm", type="primary"):
            if need_striker:
                inn["batsmen"]["striker"] = {"name":nb,"runs":0,"balls":0,"fours":0,"sixes":0}
            else:
                inn["batsmen"]["non_striker"] = {"name":nb,"runs":0,"balls":0,"fours":0,"sixes":0}
    ...
    if st.session_state.get("pending_wicket"):
        ...
        striker_name = inn["batsmen"]["striker"]["name"]
        non_striker_name = inn["batsmen"]["non_striker"]["name"]
        ...
        with wc3:
            ...
            ro_who = st.radio("Who got Run Out?",
                [f"Striker ({striker_name})", f"Non-Striker ({non_striker_name})"],
                key="ro_who", horizontal=True)
            if st.button("🏃 Run Out",use_container_width=True,key="wro"):
                dismissal = f"run out ({ro_fielder})"
                run_out_non_striker = "Non-Striker" in ro_who
        with wc4:
        ...
        if dismissal:
            ...
            if run_out_non_striker:
                # Non-striker run out — dismiss them directly, striker stays
                ns_name = non_striker_name
                if ns_name not in inn["batting_card"]:
                    inn["batting_card"][ns_name] = {"name":ns_name,"runs":0,"balls":0,"fours":0,"sixes":0,"out":False}
                inn["batting_card"][ns_name]["out"] = True
                inn["batting_card"][ns_name]["dismissal"] = dismissal
                # Increment wicket manually (don't pass through record_ball wicket logic)
                inn["wickets"] += 1
                inn["bowler"]["wickets"] += 1
                inn["fall_of_wickets"].append({"wicket":inn["wickets"],"score":inn["score"],
                    "overs":ostr(inn["balls"]),"batsman":ns_name})
                # Clear non-striker slot — new batsman will come as non-striker
                inn["batsmen"]["non_striker"] = {"name":"","runs":0,"balls":0,"fours":0,"sixes":0}
                # Record the ball without wicket (striker didn't get out)
                record_ball(inn, 0, wicket=False)
            else:
                sname = striker_name
    ...
    st.markdown("**Overthrow (bat runs + OT extras):**")
    ot_cols = st.columns(5)
    with ot_cols[0]:
        if st.button("0+OT1\n=1",use_container_width=True,key="ot01"):
            record_ball(inn,0,extra="overthrow",ot_runs=1); push(); st.rerun()
    with ot_cols[1]:
        if st.button("0+OT2\n=2",use_container_width=True,key="ot02"):
            record_ball(inn,0,extra="overthrow",ot_runs=2); push(); st.rerun()
    with ot_cols[2]:
        if st.button("0+OT3\n=3",use_container_width=True,key="ot03"):
            record_ball(inn,0,extra="overthrow",ot_runs=3); push(); st.rerun()
    with ot_cols[3]:
        if st.button("0+OT4\n=4",use_container_width=True,key="ot04"):
            record_ball(inn,0,extra="overthrow",ot_runs=4); push(); st.rerun()
    with ot_cols[4]:
        if st.button("1+OT1\n=2",use_container_width=True,key="ot11"):
            record_ball(inn,1,extra="overthrow",ot_runs=1); push(); st.rerun()

    ot_cols2 = st.columns(5)
    with ot_cols2[0]:
        if st.button("1+OT2\n=3",use_container_width=True,key="ot12"):
            record_ball(inn,1,extra="overthrow",ot_runs=2); push(); st.rerun()
    with ot_cols2[1]:
        if st.button("1+OT3\n=4",use_container_width=True,key="ot13"):
            record_ball(inn,1,extra="overthrow",ot_runs=3); push(); st.rerun()
    with ot_cols2[2]:
        if st.button("1+OT4\n=5",use_container_width=True,key="ot14"):
            record_ball(inn,1,extra="overthrow",ot_runs=4); push(); st.rerun()
    with ot_cols2[3]:
        if st.button("2+OT2\n=4",use_container_width=True,key="ot22"):
            record_ball(inn,2,extra="overthrow",ot_runs=2); push(); st.rerun()
    with ot_cols2[4]:
        if st.button("2+OT3\n=5",use_container_width=True,key="ot23"):
            record_ball(inn,2,extra="overthrow",ot_runs=3); push(); st.rerun()

    ot_cols3 = st.columns(3)
    with ot_cols3[0]:
        if st.button("2+OT4\n=6",use_container_width=True,key="ot24"):
            record_ball(inn,2,extra="overthrow",ot_runs=4); push(); st.rerun()
    with ot_cols3[1]:
        if st.button("3+OT3\n=6",use_container_width=True,key="ot33"):
            record_ball(inn,3,extra="overthrow",ot_runs=3); push(); st.rerun()
    with ot_cols3[2]:
        if st.button("3+OT4\n=7",use_container_width=True,key="ot34"):
            record_ball(inn,3,extra="overthrow",ot_runs=4); push(); st.rerun()
...
def page_result(s):
    ...
    pstats = get_player_match_stats(s, s["match_no"])
    if pstats:
        mom_name = max(pstats, key=lambda p: pstats[p]["impact"])
        mom_d = pstats[mom_name]
        mom_b64 = get_b64(f"{pkey(mom_name)}.jpg")
        mom_img = (f'<img src="data:image/jpeg;base64,{mom_b64}" style="border-radius:50%;width:100px;height:100px;'
                   f'object-fit:cover;border:3px solid #f9a825;display:block;margin:auto;">'
                   if mom_b64 else
                   f'<div style="width:100px;height:100px;border-radius:50%;background:#2a2a3e;'
                   f'display:flex;align-items:center;justify-content:center;font-size:28px;'
                   f'font-weight:700;color:#f9a825;border:3px solid #f9a825;margin:auto;">'
                   f'{"".join([x[0] for x in mom_name.split()[:2]])}</div>')
        st.markdown(
            f'<div style="background:linear-gradient(135deg,#1a1a2e,#0d1b2a);border:2px solid #f9a825;'
            f'border-radius:16px;padding:20px;text-align:center;margin:16px 0;">'
            f'<div style="font-family:Bebas Neue,sans-serif;font-size:1rem;color:#aaa;letter-spacing:3px;'
            f'margin-bottom:12px;">⭐ MAN OF THE MATCH</div>'
            f'{mom_img}'
            f'<div style="font-family:Bebas Neue,sans-serif;font-size:1.8rem;color:#f9a825;margin-top:10px;">'
            f'{mom_name}</div>'
            f'<div style="font-family:Rajdhani,sans-serif;font-size:1rem;color:#aaa;">{mom_d["team"]}</div>'
            f'<div style="font-family:Rajdhani,sans-serif;font-size:1.1rem;color:#fff;margin-top:6px;">'
            f'🏏 {mom_d["runs"]} runs &nbsp;·&nbsp; 🎳 {mom_d["wickets"]} wickets</div>'
...
def page_tournament():
    ...
    for stage in ["League","Qualifier","FINAL"]:
        ...
        st.markdown(f"**{label}**")
        for m in sm:
            res   = m["result"] or "🕐 Upcoming"
            rc    = "#4caf50" if m["result"] else "#888"
            bg    = "#1a2a1a" if m["result"] else "#1a1a2e"
            day   = m.get("day","")
            mom_name = ""
            match_result_row = None
            if m["result"]:
                for mr in s.get("match_results",[]):
                    if mr.get("Match No")==m["match_no"]:
                        mom_name = mr.get("Man of the Match","")
                        match_result_row = mr
                        break
            mom_info = f' &nbsp;·&nbsp; <span style="color:#f9a825;">⭐ {mom_name}</span>' if mom_name else ""

            # Match row header
            st.markdown(
                f'<div style="background:{bg};border:1px solid #333;border-radius:8px;padding:10px 16px;margin:3px 0 0 0;font-family:Rajdhani,sans-serif;display:flex;align-items:center;gap:12px;">' 
                ...
                f'<span style="color:{rc};min-width:120px;text-align:right;">{res}</span></div>', unsafe_allow_html=True)

            # Scorecard expander — only for completed matches
            if m["result"]:
                i1d = s.get("_saved_inn1_"+str(m["match_no"])) or {}
                i2d = s.get("_saved_inn2_"+str(m["match_no"])) or {}
                if i1d or i2d:
                    with st.expander(f"📋 View Scorecard — M{m['match_no']}"):
                        # MOM banner inside expander
                        if mom_name:
                            mb = get_b64(f"{pkey(mom_name)}.jpg")
                            mimg = (f'<img src="data:image/jpeg;base64,{mb}" style="border-radius:50%;width:50px;height:50px;object-fit:cover;border:2px solid #f9a825;vertical-align:middle;margin-right:10px;">' if mb
                                    else f'<span style="display:inline-block;width:50px;height:50px;border-radius:50%;background:#2a2a3e;border:2px solid #f9a825;text-align:center;line-height:50px;font-weight:700;color:#f9a825;vertical-align:middle;margin-right:10px;">{"".join([x[0] for x in mom_name.split()[:2]])}</span>')
                            st.markdown(
                                f'<div style="background:#1a1a2e;border:1px solid #f9a82566;border-radius:8px;padding:10px 16px;margin-bottom:12px;display:flex;align-items:center;">' 
                                f'<span style="font-family:Rajdhani,sans-serif;font-size:.8rem;color:#aaa;margin-right:12px;">⭐ MAN OF THE MATCH</span>'
                                f'{mimg}<span style="font-family:Rajdhani,sans-serif;font-weight:700;color:#f9a825;">{mom_name}</span></div>',
                                unsafe_allow_html=True)
                        sc1, sc2 = st.columns(2)
                        if i1d:
                            with sc1:
                                render_card(i1d, i1d.get("batting_team",""), i1d.get("bowling_team",""))
                        if i2d:
                            with sc2:
                                render_card(i2d, i2d.get("batting_team",""), i2d.get("bowling_team",""))
