...
def record_ball(inn, runs, extra=None, wicket=False):
    ...
    inn["ball_log"].append({"runs":runs,"extra":extra,"wicket":wicket})
    sync_card(inn)
...
def undo_ball(inn):
    ...
    if legal:
        ...
        if not b["wicket"]: inn["batsmen"]["striker"]["balls"]=max(0,inn["batsmen"]["striker"]["balls"]-1)
    if b["wicket"] and b["extra"]!="wide":
        inn["wickets"]=max(0,inn["wickets"]-1)
        if inn["fall_of_wickets"]: inn["fall_of_wickets"].pop()
    if not b["wicket"] and b["extra"] not in ("wide",):
        inn["batsmen"]["striker"]["runs"]=max(0,inn["batsmen"]["striker"]["runs"]-b["runs"])
        if b["runs"]==4: inn["batsmen"]["striker"]["fours"]=max(0,inn["batsmen"]["striker"]["fours"]-1)
        elif b["runs"]==6: inn["batsmen"]["striker"]["sixes"]=max(0,inn["batsmen"]["striker"]["sixes"]-1)
    sync_card(inn); return True
...
.ball{display:inline-block;width:34px;height:34px;border-radius:50%;text-align:center;line-height:34px;font-weight:700;font-size:.85rem;margin:2px;}
...
/* ── Popups ── */
.popup-overlay{position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:9999;
  display:flex;align-items:center;justify-content:center;
  background:rgba(0,0,0,0.78);animation:fadeInOut 5s forwards;}
@keyframes fadeInOut{0%{opacity:0}10%{opacity:1}75%{opacity:1}100%{opacity:0;pointer-events:none;}}
.popup-wicket{text-align:center;}
.out-text{font-family:'Bebas Neue',sans-serif;font-size:10rem;color:#ff1744;
  text-shadow:0 0 40px #ff1744,0 0 80px #ff000088;letter-spacing:10px;line-height:1;
  animation:shakeIn 0.35s ease-out;}
.out-sub{font-family:'Rajdhani',sans-serif;font-size:2rem;color:#fff;letter-spacing:4px;}
@keyframes shakeIn{0%{transform:scale(0.2) rotate(-8deg)}60%{transform:scale(1.12) rotate(3deg)}100%{transform:scale(1) rotate(0)}}
.popup-four{text-align:center;}
.four-text{font-family:'Bebas Neue',sans-serif;font-size:13rem;color:#f9a825;
  text-shadow:0 0 50px #f9a825,0 0 100px #f9a82566;letter-spacing:6px;line-height:1;
  animation:popIn 0.3s cubic-bezier(.68,-0.55,.27,1.55);}
.four-sub{font-family:'Rajdhani',sans-serif;font-size:2rem;color:#fff;letter-spacing:4px;}
@keyframes popIn{0%{transform:scale(0)}80%{transform:scale(1.18)}100%{transform:scale(1)}}
.popup-duck{text-align:center;}
.duck-text{font-family:'Bebas Neue',sans-serif;font-size:5rem;color:#f9a825;letter-spacing:4px;margin-top:8px;}
.duck-sub{font-family:'Rajdhani',sans-serif;font-size:1.4rem;color:#aaa;margin-top:4px;}
...
# ═══════════════════════════════════════════════════════════════════════════════
def render_popup():
    """Render wicket/four/duck popup if triggered"""
    popup = st.session_state.get("popup_type")
    if not popup:
        return
    ...
    cel_b64 = img_b64(str(PHOTOS_DIR / "wicket_celebration.png"))
    duck_path = str(PHOTOS_DIR / "Sad_Cricket_Duck_Animation.mp4")

    if popup == "wicket":
        cel_html = (f'<img src="{cel_b64}" style="height:300px;object-fit:contain;filter:drop-shadow(0 0 30px #ff1744);">'
                    if cel_b64 else '')
        html = f"""<div class="popup-overlay" id="popup-overlay">
          <div class="popup-wicket">
            {cel_html}
            <div class="out-text">OUT!</div>
            <div class="out-sub">⚡ WICKET TAKEN ⚡</div>
          </div>
        </div>
        <script>
          setTimeout(()=>{{
        ...
        try:
            with open(duck_path) as f: duck_svg = f.read()
        except: duck_svg = ""
        html = f"""<div class="popup-overlay" id="popup-overlay">
          <div class="popup-duck">
            <div style="transform:scale(1.8);display:inline-block;">{duck_svg}</div>
            <div class="duck-text">🦆 DUCK!</div>
            <div class="duck-sub">OUT FOR ZERO · WALKING BACK SADLY</div>
          </div>
        </div>
        <script>
          setTimeout(()=>{{
        ...
        html = """<div class="popup-overlay" id="popup-overlay">
          <div class="popup-four">
            <div style="font-size:5rem;line-height:1;">🏏</div>
            <div class="four-text">4</div>
            <div class="four-sub">✦ BOUNDARY ✦</div>
          </div>
        </div>
        <script>
          setTimeout(()=>{{
    ...
    # Auto-clear popup after 3s by scheduling rerun
    time.sleep(3)
...
def page_live():
    ...
    if st.session_state.get("popup_type"):
        render_popup()
        return
    ...
    if inn["ball_log"]:
        ...
        for b in inn["ball_log"][-36:]:
            ...
            elif b["extra"]=="wide": cls,lbl="bWd","Wd"
            elif b["extra"]=="noball": cls,lbl="bNb","Nb"
            else:
                r=b["runs"]; cls=f"b{r}" if r in [0,1,2,3,4,6] else "b1"; lbl=str(r)
    ...
    if striker["name"]=="":
        ...
        batted=set(inn["batting_card"].keys())
        avail=[p for p in TEAMS[bt]["players"] if p not in batted]
        if not avail: st.error("All out!"); page_end_innings(s,ik); return
    ...
    for col,lbl,val in zip(rcols,["0","1","2","3","4 ✦","W 🎯"],[0,1,2,3,4,"W"]):
        with col:
            if st.button(lbl,use_container_width=True,key=f"r{val}"):
                if val=="W":
                    striker_runs = inn["batsmen"]["striker"]["runs"]
                    st.session_state["pending_wicket"] = True
                    st.session_state["striker_runs_before_out"] = striker_runs
                else:
                    if val==4:
                        record_ball(inn,4)
                        st.session_state["popup_type"] = "four"
                    else:
                        record_ball(inn,val)
                        st.session_state.pop("popup_type", None)
                push(); st.rerun()
    ...
    if st.session_state.get("pending_wicket"):
        ...
        if dismissal:
            ...
            if striker_runs == 0:
                st.session_state["popup_type"] = "duck"
            else:
                st.session_state["popup_type"] = "wicket"
...
def render_card(inn,bt,bwt):
    ...
    for n,d in inn["bowling_card"].items():
        balls=d["overs"]*6+d.get("balls",0)
        ec=round((d["runs"]/balls)*6,2) if balls>0 else 0
        brows.append({"Bowler":n,"O":d["overs"],"R":d["runs"],"W":d["wickets"],"Econ":ec})
    if brows: st.table(brows)
...
def page_tournament():
    ...
    df=pd.DataFrame([{
        "#":medals[i],"Team":t,
        "Played":d["P"],"Won":d["W"],"Lost":d["L"],"Tied":d["T"],"Points":d["Pts"],
        "NRR":round(d.get("NRR",0.0),3)
    } for i,(t,d) in enumerate(rows)])
    st.dataframe(df,use_container_width=True,hide_index=True,column_config={
        "#":st.column_config.TextColumn("",width="small"),
        "Team":st.column_config.TextColumn("Team",width="large"),
        "Played":st.column_config.NumberColumn("Played",width="small"),
        "Won":st.column_config.NumberColumn("Won",width="small"),
        "Lost":st.column_config.NumberColumn("Lost",width="small"),
        "Tied":st.column_config.NumberColumn("Tied",width="small"),
        "Points":st.column_config.NumberColumn("Points",width="small"),
        "NRR":st.column_config.NumberColumn("NRR",width="small",format="%.3f")})
