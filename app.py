...
def record_ball(inn, runs, extra=None, wicket=False):
    ...
    bye_type = extra in ("bye","legbye","overthrow")
...
def page_live():
    ...
    ec3 = st.columns(4)
    with ec3[0]:
        if st.button("OT +1",use_container_width=True,key="eTB1",help="Overthrow — 1 run to extras, not to batsman"):
            record_ball(inn,1,extra="overthrow"); push(); st.rerun()
    with ec3[1]:
        if st.button("OT +2",use_container_width=True,key="eTB2",help="Overthrow — 2 runs to extras"):
            record_ball(inn,2,extra="overthrow"); push(); st.rerun()
    with ec3[2]:
        if st.button("OT +3",use_container_width=True,key="eTB3",help="Overthrow — 3 runs to extras"):
            record_ball(inn,3,extra="overthrow"); push(); st.rerun()
    with ec3[3]:
        if st.button("OT +4 🏏",use_container_width=True,key="eTB4",help="Overthrow boundary — 4 runs to extras"):
            record_ball(inn,4,extra="overthrow"); push(); st.rerun()
